...
struct PetAnimationView: NSViewRepresentable {
    ...
    static func buildHTML(initialStatus: String) -> String {
        ...
        let svgNames = ["resting", "working", "compacting", "questioning", "socket-face"]
        var divs = ""
        for name in svgNames {
            let activeClass = name == initialStatus ? " active" : ""
            let svg = PetSVGContent.svgContent(for: name)
            divs += "<div id=\"\(name)\" class=\"pet\(activeClass)\">\(svg)</div>\n"
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        function setStatus(name) {
            document.querySelectorAll('.pet').forEach(function(el) {
